...
public class PhysicsPoser : MonoBehaviour
{
    ...
    [Range(0, 1)]
    public float slowDownAngularVelocity = 0.75f;

    [Range(0, 100)]
    public float maxPositionChange = 75.0f;

    [Range(0, 100)]
    public float maxRotationChange = 75.0f;
    ...
    private void UpdateTracking(InputDevice inputDevice)
    {
        ...
        inputDevice
            .TryGetFeatureValue(CommonUsages.devicePosition,
            out targetPosition);
        inputDevice
            .TryGetFeatureValue(CommonUsages.deviceRotation,
            out targetRotation);
    }
    ...
    private void MoveUsingPhysics()
    {
        ...
        if (IsValidVelocity(velocity.x))
        {
            ...
            rigidBody.velocity =
                Vector3.MoveTowards(rigidBody.velocity, velocity, maxChange);
        }
    }
    ...
    private void RotateUsingPhysics()
    {
        ...
        if (IsValidVelocity(angularVelocity.x))
        {
            ...
            rigidBody.angularVelocity =
                Vector3
                    .MoveTowards(GetComponent<Rigidbody>().angularVelocity,
                    angularVelocity,
                    maxChange);
        }
    }
    ...
    private Vector3 FindNewAngularVelocity()
    {
        ...
        Quaternion difference =
            targetRotation * Quaternion.Inverse(rigidBody.rotation);
        difference
            .ToAngleAxis(out float angleInDegrees, out Vector3 rotationAxis);
